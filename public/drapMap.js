...
/**
 * The colour ramp, keyed to the marine SSB band edges. A copy of DRAP_STOPS
 * in src/tiles.ts, because a browser cannot import the TypeScript;
 * test/drap-map.test.ts fails if the copy drifts. Same reason, same shape as
 * MARINE_SSB_BAND_EDGES_HZ in hf.js.
 */
export const DRAP_STOPS = [
  [2.045, 90, 200, 255],
  [4.0, 80, 235, 190],
  [6.2, 150, 240, 120],
  [8.1, 240, 226, 70],
...
const FLOOR_MHZ = DRAP_STOPS[0][0]
const TOP_MHZ = DRAP_STOPS[DRAP_STOPS.length - 1][0]

/**
 * `drapColor` in src/tiles.ts, as an rgba() string. Transparent below the
 * lowest marine band: a cutoff under 2.045 MHz absorbs nothing anyone here is
 * transmitting on, and painting it would colour most of a quiet globe.
 */
export function drapColor(mhz) {
  if (!Number.isFinite(mhz) || mhz < FLOOR_MHZ) return 'rgba(0,0,0,0)'
  const value = Math.min(mhz, TOP_MHZ)
  let seg = 0
  ...
  const fraction = (value - FLOOR_MHZ) / (TOP_MHZ - FLOOR_MHZ)
  const r = Math.round(ar + (br - ar) * t)
  const g = Math.round(ag + (bg - ag) * t)
  const b = Math.round(ab + (bb - ab) * t)
  // Quantised to the same 0-255 step src/tiles.ts writes into a PNG, so the
  // webapp and the chart overlay are the same colour rather than nearly.
  const alpha = Math.round(
    255 * (0.3 + 0.55 * Math.min(1, Math.max(0, fraction)))
  )
...
export function drawDrapMap(canvas, grid, options = {}) {
  ...
  for (let row = 0; row < grid.frequenciesMHz.length; row++) {
    ...
    for (let col = 0; col < cells.length; col++) {
      const color = drapColor(cells[col])
