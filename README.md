...
It sits under `environment.noaa.swpc.kp.forecast.outlook27`, a third horizon
alongside `max24h` and `max72h`: the outlook is the same index further out, and
asking "what is the worst Kp coming" should not require knowing which NOAA
product answers. Its own node rather than more siblings, because the two are not
the same kind of data — one is a 3-hourly sample, the other a whole-day maximum
— and read as a single series they would misrepresent both.
