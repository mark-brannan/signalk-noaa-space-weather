...
//
// Products are copied out of the space-weather package's dist/ under plugin/
// and loaded unbundled; public/ is that package's page, synced here by
// scripts/sync-webapp.mjs on prepare, so `npm install` has to have run first.
...
// Resolved through Node rather than spelt as node_modules/space-weather, so a
// linked or hoisted install lands in the same place the runtime would load.
export const CORE = path.dirname(
  createRequire(import.meta.url).resolve('space-weather/package.json')
)
export const DIST = path.join(CORE, 'dist')
...
export function defineSite({
  ...
  function readSite(name) {
    ...
    // Named rather than left as ENOENT: a missing plugin/ file means the core
    // package is not installed, which is one command away and nothing like a
    // broken import.
    if (name.startsWith(PLUGIN_PREFIX) && !fssync.existsSync(source)) {
      throw new Error(
        `${name} is missing from ${DIST} -- run \`npm install\` first; the ` +
          "site's live data layer is the space-weather package"
      )
    }
  ...
  async function build() {
    // public/ is synced from the package by sync-webapp.mjs on prepare;
    ...
    if (!fssync.existsSync(path.join(DIST, 'browser', 'live.js'))) {
      console.error('space-weather is not installed -- run `npm install` first')
      process.exit(1)
    }
    ...
    console.log(
      `assembled ${path.relative(REPO, OUT)}/ (${SITE_FILES.length} files, ` +
        `${PLUGIN_MODULES.length} of them the space-weather core)`
    )
    return SITE_FILES
  }
