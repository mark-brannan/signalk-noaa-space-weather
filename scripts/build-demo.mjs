...
// './signalk.js' to the snapshot-backed version and run unchanged against
// demo/snapshot.json instead of a Signal K server. The demo's own framing
// (what this page is, when it was captured, where to get the real thing) is
// appended as one script tag rather than edited in, which is what keeps
// index.html unforked.
...
export const sourceOf = (name) =>
  DEMO_FILES[name] ? path.join(DEMO, DEMO_FILES[name]) : path.join(PUBLIC, name)
...
const readSite = (name) => fssync.readFileSync(sourceOf(name), 'utf8')
...
// The subset copied out of public/: every file the page reaches that demo/
// does not replace, the vendored coast-wright subtree included.
export const PUBLIC_MODULES = SITE_FILES.filter((name) => !DEMO_FILES[name])
